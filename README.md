...
A small RAG (Retrieval-Augmented Generation) application: upload PDF/TXT documents, ask
questions about them in plain English, and get answers grounded in the uploaded content —
with sources and a confidence indicator, and a clear "I don't know" when the answer isn't
in the documents.

## 1. Problem Understanding

Users often have several documents (policies, handbooks, reports) and want a direct answer
to a specific question without reading through all of them manually. A plain LLM chatbot
can't answer questions about private documents it was never trained on, and if pushed, it
may confidently invent an answer. This project solves both problems with a RAG pipeline:
retrieve the most relevant passages from the user's own documents first, then ask the LLM
to answer strictly from that retrieved context — and explicitly refuse when nothing
relevant was found.

## 2. Architecture

See [`architecture.md`](./architecture.md) for the diagram. In short:

```
User → Streamlit UI → Document Parser (pypdf) → Chunker → Embeddings
(sentence-transformers) → ChromaDB (vector store) → Retrieval → Confidence
check → Prompt with context → Groq API (Llama 3.3 70B) → Answer + Sources → UI
```

- **Document Parser** (`backend/document_processor.py`) extracts text per page from PDFs
  (pypdf) or reads TXT files directly.
...
- **LLM Client** (`backend/llm_client.py`) is a thin wrapper around the Groq API (OpenAI-compatible
  chat completions, running Llama 3.3 70B by default).
...
| pypdf | Lightweight, no external service, good enough text extraction for typical PDFs |
...
| Groq API (Llama 3.3 70B) | Free API key, very fast inference (Groq's custom LPU hardware), OpenAI-compatible SDK — easy to explain and demo without a paid account |
...
2. **Prompt-level grounding:** the system prompt in `backend/qa.py` explicitly instructs
   Claude to answer only from the given context, to say the exact "not found" sentence
...
unanswerable question ("What is the company's stock price?").
...
pipeline structure, reviewing the chunking/overlap logic, and drafting this README. The
LLM-answering component itself calls the Groq API (Llama 3.3 70B) at runtime. All
generated code was read, tested against the sample documents, and adjusted by hand
(e.g. the retrieval-gate threshold and prompt wording were tuned manually after testing).
...
| Document processing + chunking | 1 hr |
...
| Testing & debugging with sample documents | 1 hr |
